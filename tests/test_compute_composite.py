...
def test_normalize_recruiting_talent_scales_and_caps():
    assert normalize_recruiting_talent(0) == 0
    assert normalize_recruiting_talent(10) == 2.0
    assert normalize_recruiting_talent(100) == 10.0
    assert normalize_recruiting_talent(-100) == -10.0
...
def test_compute_composite_worked_example_miami_favored():
    ...
    # points of returning-snap-share differential, +10 points of
    # recruiting-rating differential.
    result = compute_composite(
        weight_diff_lbs=25, sp_plus_gap=10, experience_diff_pct=20,
        recruiting_rating_diff=10, weights=WEIGHTS,
    )
    assert result.mass == 2.5
    assert result.push == 2.0
    assert result.experience == 2.0
    assert result.recruiting == 2.0
    # 0.3*2.5 + 0.3*2.0 + 0.2*2.0 + 0.2*2.0 = 0.75 + 0.6 + 0.4 + 0.4 = 2.15
    assert round(result.composite, 4) == 2.15
    assert result.verdict == "slight-to-moderate edge"


def test_compute_composite_clamps_extreme_inputs():
    result = compute_composite(
        weight_diff_lbs=1000, sp_plus_gap=1000, experience_diff_pct=1000,
        recruiting_rating_diff=1000, weights=WEIGHTS,
    )
    assert result.mass == 10.0
    assert result.push == 10.0
    assert result.experience == 10.0
    assert result.recruiting == 10.0
...
def test_compute_composite_weights_sum_need_not_be_enforced_here():
    ...
    skewed = {"mass": 1.0, "push": 1.0, "experience": 1.0, "recruiting": 1.0}
    result = compute_composite(10, 10, 100, 100, skewed)
    assert result.composite == 10.0  # clamped, not 23.0
